...
import org.opensearch.sql.spark.scheduler.AsyncQuerySchedulerJob;
 ...
 * OpenSearch has not invoke plugins' createComponents() method. That is saying the plugin is not
 ...
 * plugin.
 *
 * <p>This sample job runner takes the "indexToWatch" from job parameter and logs that index's
 * shards.
 */
public class OpenSearchRefreshIndexJob implements ScheduledJobRunner, AsyncQuerySchedulerJob {
            ...
            lockService.acquireLock(
                ...
                ActionListener.wrap(
                    lock -> {
                      ...
                      OpenSearchRefreshIndexJobRequest parameter =
  ...
  private void runTaskForIntegrationTests(OpenSearchRefreshIndexJobRequest jobParameter) {
    this.client.index(
        new IndexRequest(jobParameter.getIndexToWatch())
            .id(UUID.randomUUID().toString())
            .source("{\"message\": \"message\"}", XContentType.JSON));
  }

  private void runTaskForLockIntegrationTests(OpenSearchRefreshIndexJobRequest jobParameter)
