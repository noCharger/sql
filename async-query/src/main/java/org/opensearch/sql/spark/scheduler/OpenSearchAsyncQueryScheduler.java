...
public class OpenSearchAsyncQueryScheduler {
  ...
  public void scheduleJob(AsyncQuerySchedulerRequest request) {
    assertRequestType(request);
    OpenSearchScheduleQueryJobRequest osRequest = (OpenSearchScheduleQueryJobRequest) request;
    ...
    indexRequest.id(osRequest.getName());
    indexRequest.opType(DocWriteRequest.OpType.CREATE);
    indexRequest.setRefreshPolicy(WriteRequest.RefreshPolicy.IMMEDIATE);
    IndexResponse indexResponse;
    try {
      indexRequest.source(osRequest.toXContent(JsonXContent.contentBuilder(), EMPTY_PARAMS));
      ...
      throw new IllegalArgumentException("A job already exists with name: " + osRequest.getName());
    } catch (Throwable e) {
      LOG.error("Failed to schedule job : {}", osRequest.getName(), e);
      throw new RuntimeException(e);
    }

    if (indexResponse.getResult().equals(DocWriteResponse.Result.CREATED)) {
      LOG.debug("Job : {}  successfully created", osRequest.getName());
    } else {
  ...
  public void updateJob(AsyncQuerySchedulerRequest request) throws IOException {
    assertRequestType(request);
    OpenSearchScheduleQueryJobRequest osRequest = (OpenSearchScheduleQueryJobRequest) request;
    assertIndexExists();
    UpdateRequest updateRequest = new UpdateRequest(SCHEDULER_INDEX_NAME, osRequest.getName());
    updateRequest.setRefreshPolicy(WriteRequest.RefreshPolicy.IMMEDIATE);
    updateRequest.doc(osRequest.toXContent(JsonXContent.contentBuilder(), EMPTY_PARAMS));
    ...
    try {
      ...
      throw new IllegalArgumentException("Job: " + osRequest.getName() + " doesn't exist");
    } catch (Throwable e) {
      LOG.error("Failed to update job : {}", osRequest.getName(), e);
      throw new RuntimeException(e);
    }
    ...
    if (updateResponse.getResult().equals(DocWriteResponse.Result.UPDATED)
      ...
      LOG.debug("Job : {} successfully updated", osRequest.getName());
    } else {
      throw new RuntimeException(
