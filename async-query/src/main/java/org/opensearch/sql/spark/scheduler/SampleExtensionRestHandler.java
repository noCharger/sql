 ...
 *
 * <p>Users need to provide "id", "index", "job_name", and "interval" parameter to schedule a job.
 * e.g. {@code POST /_plugins/scheduler_sample/watch?id=dashboards-job-id&job_name=watch dashboards
 * index&index=.opensearch_dashboards_1&interval=1 }
 *
 * <p>creates a job with id "dashboards-job-id" and job name "watch dashboards index", which logs
 * ".opensearch_dashboards_1" index's shards info every 1 minute
 *
 * <p>Users can remove that job by calling {@code DELETE
 * /_plugins/scheduler_sample/watch?id=dashboards-job-id}
 */
public class SampleExtensionRestHandler extends BaseRestHandler {
  ...
  protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client)
    ...
    if (request.method().equals(RestRequest.Method.POST)) {
      // compose OpenSearchRefreshIndexJobRequest object from request
      ...
      Long lockDurationSeconds =
          lockDurationSecondsString != null ? Long.parseLong(lockDurationSecondsString) : null;
      String jitterString = request.param("jitter");
      Double jitter = jitterString != null ? Double.parseDouble(jitterString) : null;

      if (id == null || indexName == null || jobName == null || interval == null) {
        throw new IllegalArgumentException(
            "Must specify id, index, job_name, and interval parameters");
      }

      OpenSearchRefreshIndexJobRequest.Builder builder =
          new OpenSearchRefreshIndexJobRequest.Builder()
              .withJobName(jobName)
              .withIndexToWatch(indexName)
              .withSchedule(String.format("PT%sM", interval))
              .withEnabled(true)
              .withEnabledTime(Instant.now())
              .withLastUpdateTime(Instant.now());

      if (lockDurationSeconds != null) {
        builder.withLockDurationSeconds(lockDurationSeconds);
      }
      ...
      OpenSearchRefreshIndexJobRequest jobParameter = builder.build();

      IndexRequest indexRequest =
          new IndexRequest()
              .index(OpenSearchAsyncQueryScheduler.SCHEDULER_INDEX_NAME)
              .id(id)
      ...
      String id = request.param("id");
      if (id == null) {
        throw new IllegalArgumentException("Must specify id parameter");
      }
      DeleteRequest deleteRequest =
          new DeleteRequest().index(OpenSearchAsyncQueryScheduler.SCHEDULER_INDEX_NAME).id(id);
